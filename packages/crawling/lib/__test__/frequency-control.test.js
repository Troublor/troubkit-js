...
const frequency_control_1 = require("../frequency-control");
const tools_1 = require("@troubkit/tools");
describe("FrequencyControlledTaskManager", () => {
  let manager;
  beforeEach(() => {
    manager = new frequency_control_1.FrequencyControlledTaskManager(100, 1);
  });
  afterEach(() => {
    manager.stop();
  });
  ...
  test("should return void if callback is provided", () => {
    ...
    return Promise.all([
      new Promise(resolve => {
        const ret = manager.addTask(() => 1, (err, result) => {
      ...
      new Promise(resolve => {
        const ret = manager.addTask(() => {
          ...
          expect(err === null || err === void 0 ? void 0 : err.message).toEqual("err");
  ...
  test("should return a promise if callback is not provided", async () => {
    manager.start();
    const aPromise = manager.addTask(() => 1);
    expect(aPromise).toBeInstanceOf(Promise);
    await expect(aPromise).resolves.toBe(1);
    const promise = manager.addTask(() => new Promise(r => r(1)));
    expect(promise).toBeInstanceOf(Promise);
    await expect(promise).resolves.toBe(1);
    const rejectPromise = manager.addTask(() => Promise.reject(new Error("err")));
    await expect(rejectPromise).rejects.toThrow("err");
  });
});
//# sourceMappingURL=frequency-control.test.js.map
